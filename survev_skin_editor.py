...
with st.sidebar:
    ...
    bp_c2 = None
    if bp_fill != "Solid":
        bp_c2 = st.color_picker("Secondary color", "#8B4513", key="bp_c2")
    bp_pattern = st.selectbox("Pattern", ["None","Stripes","Spots","Custom"], key="bp_pattern")
    bp_custom = None
    if bp_pattern == "Custom":
        bp_custom = st.file_uploader("Upload tile PNG", type="png", key="bp_patup")
    ...
    bd_c2 = None
    if bd_fill != "Solid":
        bd_c2 = st.color_picker("Secondary color", "#FFC071", key="bd_c2")
    bd_pattern = st.selectbox("Pattern", ["None","Stripes","Spots","Custom"], key="bd_pattern")
    bd_custom = None
    if bd_pattern == "Custom":
        bd_custom = st.file_uploader("Upload tile PNG", type="png", key="bd_patup")
    ...
    h_c2 = None
    if h_fill != "Solid":
        h_c2 = st.color_picker("Secondary color", "#8B4513", key="h_c2")
    h_pattern = st.selectbox("Pattern", ["None","Stripes","Spots","Custom"], key="h_pattern")
    h_custom = None
    if h_pattern == "Custom":
        h_custom = st.file_uploader("Upload tile PNG", type="png", key="h_patup")
